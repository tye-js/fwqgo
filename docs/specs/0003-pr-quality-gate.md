...
- Node.js 24。
- 使用 Bun cache。
- 依赖安装命令为 `bun install --frozen-lockfile`，确保 Sharp 等可选依赖与生产构建一致。
...
3. 安装依赖。
4. 执行 `bun run check`。
5. 执行 `SKIP_ENV_VALIDATION=1 bun run build`。
...
- [ ] 使用 Node.js 24 与 Bun cache。
- [ ] 执行 `bun install --frozen-lockfile`。
- [ ] 执行 `bun run check`。
- [ ] 执行 `SKIP_ENV_VALIDATION=1 bun run build`。
- [ ] 本地 `bun run lint` 通过。
- [ ] 本地 `bun run typecheck` 通过。
- [ ] 本地 `npm test` 通过。
- [ ] 本地 `SKIP_ENV_VALIDATION=1 bun run build` 通过。
...
- 同时保留 `bun run check` 与完整构建会重复执行部分 TypeScript 校验。
