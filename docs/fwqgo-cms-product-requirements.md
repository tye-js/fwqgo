...
> 当前实现状态（2026-08-23）：本文历史需求中关于“文章中提取套餐”、旧 12 Prompt、隐藏 system prompt、独立事实核查和质量修复重试的描述已退役，以当前实现状态和验证脚本为准。

## 当前实现状态

- CMS 导航按“概览 / 内容 / 套餐 / 媒体 / 运营 / 模型与接口”组织，服务器知识库独立维护。
- 文章来源、改写内容、服务器套餐和媒体资产是分离的数据域；官网套餐采集通过结构化供应商流程完成。
- 当前 Prompt 只有六个：中文正文、中文 SEO、英文正文、英文续写、英文 SEO、官网套餐采集。
- 中文正文采用 source-anchored 单轮改写，不执行独立事实核查、质量修复或自动重试；失败进入任务诊断和人工恢复流程。
...
图片最终统一使用 Sharp 库在 Bun 中处理并转换为 webp。
