...
- 管理员人工输入稳定商品键和完整返利链接；系统不再按供应商规则拼接 PID/GID，也不会从列表页自动创建多条商品链接。
- 默认 `collection.type=whmcs_product`，可额外输入独立采集地址。预览和定时任务请求 `sourceUrl ?? affiliateTargetUrl`，绝不请求站内 `/go/{slug}`。
- 当单品页拒绝服务端读取、但官网公开列表页可访问时，可显式使用 `collection.type=html_listing`；此模式必须填写独立采集地址、字段选择器和精确商品键，且必须恰好匹配一个商品，不承担套餐发现或返利链接生成。
