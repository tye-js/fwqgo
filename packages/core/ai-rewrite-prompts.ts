export const defaultChineseRewriteStylePrompt =
  "保持服务器/VPS文章的专业评测风格，只对原文做小幅改写和排版整理。保留原文中的表格、价格、配置、优惠码、官网链接和返利链接，不新增外部信息。";

export const defaultBaseRewritePrompt = `你是服务器/VPS 文章的中文编辑。请直接基于清洗后的来源原文，对正文做小幅度改写和排版整理，输出 Markdown。不执行独立事实提取或事实核查，也不要扩写、补充知识或重新创作。
...
export function buildSourceAnchoredRewritePrompt(
  ...
  const values: Record<string, string> = {
    ...
    factSheet: "已取消独立事实提取，请直接以来源原文为依据。",
    keywordPlan: "不再单独生成关键词规划。",
    rewriteLengthBudget: input.rewriteLengthBudget,
    outline: "沿用来源原文已有结构，不新增主题。",
    providerContext: "本次改写不使用供应商资料，只依据清洗后的来源原文。",
    knowledgeContext: "本次改写不使用知识库，只依据清洗后的来源原文。",
    knowledgeSections: "本次改写不添加来源之外的基础知识章节。",
    protectedContent: input.protectedContent,
    retryFeedback: "本次固定生成一次，请直接完成正文改写。",
  };
...
3. keywords 生成 2 到 6 个适合 SEO 的关键词，不要超过 6 个；关键词必须来自已完成改写的正文。
