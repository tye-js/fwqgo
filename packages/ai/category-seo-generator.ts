import { getActiveAiRewriteConfig } from "@fwqgo/ai/rewrite-config";
import { readResponseTextWithLimit } from "@fwqgo/core/bounded-response-body";
import { assertPublicHttpUrl } from "@fwqgo/core/network-url";
import { slugify } from "@fwqgo/core/utils";

import {
  buildOpenAiChatCompletionsEndpoint,
  parseAiJsonObject,
} from "./openai-compatible";

const DEFAULT_AI_REWRITE_TIMEOUT_MS = 300_000;
const CATEGORY_SEO_MAX_TOKENS = 2_000;
const MAX_AI_RESPONSE_BYTES = 512 * 1024;

type AiRewriteConfig = NonNullable<
  Awaited<ReturnType<typeof getActiveAiRewriteConfig>>
>;

type ChatCompletionResponse = {
  choices?: Array<{
    finish_reason?: string | null;
    message?: {
      content?: string | null;
    };
  }>;
  error?: {
    message?: string;
  };
};

type RawCategorySeoOutput = Partial<{
  description: string;
  keywords: unknown;
  enName: string;
  enSlug: string;
  enDescription: string;
  enKeywords: unknown;
}>;

export type CategorySeoOutput = {
  description: string;
  keywords: string[];
  enName: string;
  enSlug: string;
  enDescription: string;
  enKeywords: string[];
};

export type CategorySeoInput = {
  name: string;
  slug: string;
  description?: string | null;
  keywords?: string | null;
  enName?: string | null;
  enSlug?: string | null;
  enDescription?: string | null;
  enKeywords?: string | null;
};

function getAiRewriteTimeoutMs() {
  const configured = Number(process.env.AI_REWRITE_TIMEOUT_MS);

  if (Number.isFinite(configured) && configured >= 10_000) {
    return configured;
  }

  return DEFAULT_AI_REWRITE_TIMEOUT_MS;
}

export function getCategorySeoMaxTokens(maxTokens: number) {
  if (Number.isFinite(maxTokens) && maxTokens > 0) {
    return Math.min(Math.floor(maxTokens), CATEGORY_SEO_MAX_TOKENS);
  }

  return CATEGORY_SEO_MAX_TOKENS;
}

function createReadableError(message: string, detail?: string) {
  return new Error(detail ? `${message}；原因：${detail}` : message);
}

export function normalizeStringArray(value: unknown) {
  if (Array.isArray(value)) {
    return value.map((item) => String(item).trim()).filter(Boolean);
  }

  if (typeof value === "string") {
    return value
      .replace(/，/g, ",")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean);
  }

  return [];
}

export function normalizeEnglishSlug(value: string, fallback: string) {
  const normalized = slugify(value)
    .replace(/[^a-z0-9-]/g, "")
    .replace(/-+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 80);

  if (normalized) {
    return normalized;
  }

  return slugify(fallback)
    .replace(/[^a-z0-9-]/g, "")
    .replace(/-+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 80);
}

export function normalizeCategorySeoOutput(
  raw: RawCategorySeoOutput,
  fallback: CategorySeoInput,
) {
  const description =
    typeof raw.description === "string" && raw.description.trim()
      ? raw.description.trim().slice(0, 180)
      : "";
  const keywords = normalizeStringArray(raw.keywords).slice(0, 6);
  const enName =
    typeof raw.enName === "string" && raw.enName.trim()
      ? raw.enName.trim().slice(0, 120)
      : "";
  const enSlug =
    typeof raw.enSlug === "string" && raw.enSlug.trim()
      ? normalizeEnglishSlug(raw.enSlug, enName || fallback.slug)
      : "";
  const enDescription =
    typeof raw.enDescription === "string" && raw.enDescription.trim()
      ? raw.enDescription.trim().slice(0, 180)
      : "";
  const enKeywords = normalizeStringArray(raw.enKeywords).slice(0, 6);

  return {
    description,
    keywords,
    enName,
    enSlug,
    enDescription,
    enKeywords,
  };
}

export function validateCategorySeoOutput(output: CategorySeoOutput) {
  const issues: string[] = [];

  if (output.description.length < 60) {
    issues.push("中文 Description 不符合 SEO 摘要长度");
  }

  if (output.keywords.length < 3) {
    issues.push("中文 Keywords 少于 3 个");
  }

  if (output.enName.length < 2) {
    issues.push("英文分类为空或过短");
  }

  if (!output.enSlug) {
    issues.push("英文 slug 为空");
  }

  if (output.enDescription.length < 100) {
    issues.push("英文 Description 不符合 SEO 摘要长度");
  }

  if (output.enKeywords.length < 3) {
    issues.push("英文 Keywords 少于 3 个");
  }

  if (issues.length > 0) {
    throw createReadableError(
      "分类 SEO AI 生成失败：返回字段不符合 SEO 规范",
      issues.join("、"),
    );
  }
}

function getAiProviderErrorMessage(input: {
  status: number;
  statusText: string;
  error?: ChatCompletionResponse["error"];
}) {
  const message = input.error?.message?.trim();
  const prefix = `AI 接口请求失败：HTTP ${input.status} ${input.statusText}`;

  if (input.status === 401 || input.status === 403) {
    return `${prefix}，请检查 API Key 是否正确、是否有模型权限`;
  }

  if (input.status === 404) {
    return `${prefix}，请检查 Base URL 是否需要包含或去掉 /v1，以及模型名称是否存在`;
  }

  if (input.status === 429) {
    return `${prefix}，请求频率或额度受限，请稍后重试或更换模型`;
  }

  if (input.status >= 500) {
    return `${prefix}，服务商当前异常，请稍后重试`;
  }

  return message ? `${prefix}，${message}` : prefix;
}

async function requestCategorySeoJson(input: {
  config: AiRewriteConfig;
  endpoint: string;
  timeoutMs: number;
  maxTokens: number;
  userPrompt: string;
}) {
  const controller = new AbortController();
  let timeout: ReturnType<typeof setTimeout> | undefined;

  try {
    const request = async () => {
      const endpoint = await assertPublicHttpUrl(input.endpoint, "AI 接口地址");
      const response = await fetch(endpoint, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${input.config.apiKey}`,
          "Content-Type": "application/json",
        },
        redirect: "error",
        signal: controller.signal,
        body: JSON.stringify({
          model: input.config.model,
          temperature: input.config.temperature / 100,
          max_tokens: input.maxTokens,
          response_format: { type: "json_object" },
          messages: [
            {
              role: "system",
              content:
                "你是服务器/VPS 内容站的 SEO 运营专家。你只输出一个符合要求的 JSON 对象，不输出 Markdown、解释或额外文本。",
            },
            {
              role: "user",
              content: input.userPrompt,
            },
          ],
        }),
      });
      const responseText = await readResponseTextWithLimit(
        response,
        MAX_AI_RESPONSE_BYTES,
      );
      if (responseText === null) {
        throw createReadableError(
          "分类 SEO AI 生成失败：AI 响应过大",
          "服务商返回超过 512 KiB 安全限制，请检查中转接口或更换服务商",
        );
      }
      let data: ChatCompletionResponse | null = null;
      try {
        data = JSON.parse(responseText || "{}") as ChatCompletionResponse;
      } catch {
        // HTTP errors may legitimately return non-JSON bodies; classify them below.
      }

      return { response, data };
    };

    const timeoutPromise = new Promise<never>((_, reject) => {
      timeout = setTimeout(() => {
        controller.abort();
        reject(
          new Error(
            `分类 SEO AI 生成超时（${Math.round(input.timeoutMs / 1000)}秒）：${input.config.name} / ${input.config.model}，请稍后重试或换一个改写模型`,
          ),
        );
      }, input.timeoutMs);
    });

    const result = await Promise.race([request(), timeoutPromise]);

    if (!result.response.ok) {
      throw createReadableError(
        "分类 SEO AI 生成失败",
        getAiProviderErrorMessage({
          status: result.response.status,
          statusText: result.response.statusText,
          error: result.data?.error,
        }),
      );
    }

    if (!result.data) {
      throw createReadableError(
        "分类 SEO AI 生成失败：AI 接口返回格式错误",
        "服务商没有返回有效 JSON，请检查中转接口的 OpenAI 兼容性",
      );
    }

    const choice = result.data?.choices?.[0];
    const text = choice?.message?.content;

    if (choice?.finish_reason === "length") {
      throw createReadableError(
        "分类 SEO AI 生成失败：模型输出被截断",
        "请调大 Max Tokens，或减少批量数量后重试",
      );
    }

    if (!text) {
      throw createReadableError(
        "分类 SEO AI 生成失败：模型返回为空",
        "请检查模型名称、额度和第三方接口兼容性",
      );
    }

    return text;
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(
        `分类 SEO AI 生成超时（${Math.round(input.timeoutMs / 1000)}秒）：${input.config.name} / ${input.config.model}，请稍后重试或换一个改写模型`,
      );
    }

    throw error;
  } finally {
    if (timeout) {
      clearTimeout(timeout);
    }
  }
}

function buildCategorySeoPrompt(input: CategorySeoInput) {
  return `请为一个服务器/VPS 内容站的分类聚合页生成中英文 SEO 元信息。

分类信息：
- 中文分类名：${input.name}
- 中文 slug：${input.slug}
- 现有中文 Description：${input.description ?? ""}
- 现有中文 Keywords：${input.keywords ?? ""}
- 现有英文分类：${input.enName ?? ""}
- 现有英文 slug：${input.enSlug ?? ""}
- 现有英文 Description：${input.enDescription ?? ""}
- 现有英文 Keywords：${input.enKeywords ?? ""}

SEO 规范：
1. description 用中文，80 到 160 字，准确说明这个分类页收录的服务器/VPS/云服务内容、用户搜索意图、选型维度和适用场景。
2. description 不能堆砌关键词，不能写虚假承诺、具体价格、限时优惠或无法验证的最高级表述。
3. keywords 生成 3 到 6 个中文 SEO 关键词，使用数组，覆盖核心品类词、长尾场景词和用户检索词。
4. enName 是自然、简洁的英文分类名，符合英文站导航和分类页命名习惯。
5. enSlug 只使用小写英文字母、数字和连字符，长度 3 到 80，语义清晰，不能包含中文或拼音堆砌。
6. enDescription 用英文，120 到 160 个字符，符合英文搜索结果摘要习惯，不堆砌 keywords，不写具体价格、限时优惠或无法验证的最高级。
7. enKeywords 生成 3 到 6 个英文 SEO keywords，使用数组。
8. 只输出 JSON，不要输出代码块。

JSON 结构必须为：
{
  "description": "中文 Description",
  "keywords": ["关键词1", "关键词2", "关键词3"],
  "enName": "English category name",
  "enSlug": "english-category-slug",
  "enDescription": "English meta description",
  "enKeywords": ["keyword 1", "keyword 2", "keyword 3"]
}`;
}

export async function generateCategorySeoMetadata(
  input: CategorySeoInput,
  options: { styleId?: number } = {},
): Promise<CategorySeoOutput> {
  const config = await getActiveAiRewriteConfig(options.styleId);

  if (!config) {
    throw createReadableError(
      "分类 SEO AI 生成未启用",
      "请先在后台「内容生产 - 改写接口配置」启用一套默认配置",
    );
  }

  if (!config.apiKey) {
    throw createReadableError(
      "AI 改写配置不完整",
      `「${config.name}」缺少 API Key`,
    );
  }

  const endpoint = buildOpenAiChatCompletionsEndpoint(config.baseUrl);
  const text = await requestCategorySeoJson({
    config,
    endpoint,
    timeoutMs: getAiRewriteTimeoutMs(),
    maxTokens: getCategorySeoMaxTokens(config.maxTokens),
    userPrompt: buildCategorySeoPrompt(input),
  });
  const output = normalizeCategorySeoOutput(
    parseAiJsonObject<RawCategorySeoOutput>(text, "分类 SEO AI 生成失败"),
    input,
  );

  validateCategorySeoOutput(output);

  return output;
}
