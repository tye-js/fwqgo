    ...
    replace(
      replace(
        replace(
          coalesce(
            "basePrompt",
            $prompt$你是服务器/VPS 文章的中文编辑。请直接基于来源原文做小幅改写和排版整理，输出 Markdown。
...
只输出正文 Markdown；保留价格、配置、优惠码、链接和受保护占位符；不要引入来源之外的信息。$prompt$
          ),
          '{stylePrompt}',
          coalesce("stylePrompt", '')
        ),
        '{factSheet}',
        '已取消独立事实提取，请直接以来源原文为依据。'
      ),
      '{keywordPlan}',
      '不再单独生成关键词规划。'
    ),
...
UPDATE "ai_rewrite_configs"
SET "metadataPrompt" = replace(
  replace(
    coalesce(
      "metadataPrompt",
      $prompt$你是服务器/VPS 推广文章的 SEO 编辑。
...
UPDATE "ai_rewrite_configs"
SET "englishContentPrompt" = replace(
  coalesce(
    "englishContentPrompt",
    $prompt$Translate and localize the rewritten Chinese VPS/server article into English Markdown.
...
UPDATE "ai_rewrite_configs"
SET "englishMetadataPrompt" = replace(
  coalesce(
    "englishMetadataPrompt",
    $prompt$Generate English SEO metadata for a VPS/server article.
...
WHERE "englishMetadataPrompt" IS NOT NULL
   OR "englishMetadataStylePrompt" <> 'Write concise English SEO metadata for VPS/server deal readers. Prioritize provider name, price, specs, location, network route and buying intent. Keep the slug short and readable.';--> statement-breakpoint
...
ALTER TABLE "ai_rewrite_configs" DROP COLUMN IF EXISTS "englishMetadataStylePrompt";--> statement-breakpoint
