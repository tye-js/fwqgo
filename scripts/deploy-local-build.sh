...
web_port="$(grep -E "^WEB_PORT=" "$shared_dir/.env.production" 2>/dev/null | tail -n 1 | cut -d= -f2- | tr -d "\"'"'"'" || true)"
cms_port="$(grep -E "^CMS_PORT=" "$shared_dir/.env.production" 2>/dev/null | tail -n 1 | cut -d= -f2- | tr -d "\"'"'"'" || true)"
web_port="${web_port:-3000}"
cms_port="${cms_port:-3100}"
web_status="$(curl -sS -o /dev/null -w "%{http_code}" --max-time 15 "http://127.0.0.1:${web_port}/")"
cms_status="$(curl -sS -o /dev/null -w "%{http_code}" --max-time 15 "http://127.0.0.1:${cms_port}/")"
case "$web_status" in
  2*|3*) ;;
