...
export function ImageUpload({ onChange, value }: ImageUploadProps) {
  ...
  /**
   * 占位图不是可预览的封面（它的语义就是「没有封面」），判据统一走
   * `hasRenderableCover` —— 那里集中说明了为什么不能只用 `isRenderableImageSrc`：
   * 它对任何 `/` 开头的路径都返回 true，会把占位图放进 `next/image`，
   * 而占位图不在 `images.localPatterns` 白名单里，服务端渲染会直接抛错、整页 500。
   */
  const hasPreview = hasRenderableCover(value);
    ...
    <div className="flex min-w-0 flex-wrap items-start gap-4">
      ...
      <div className="min-w-0 space-y-4">
        <div className="space-y-2">
          ...
          />
          {value && !hasPreview && !isDefaultArticleCover(value) ? (
