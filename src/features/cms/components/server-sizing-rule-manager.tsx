...
export function ServerSizingRuleManager({ rules }: { rules: RuleRow[] }) {
  ...
  function createDraft() {
    ...
    const config = {
      ...parsed,
      versionLabel: versionLabel.trim() || parsed.versionLabel,
      status: "draft",
    } as Omit<ServerSizingRuleSet, "checksum">;
    void mutate({
  ...
  function clone(id: number) {
    void mutate({
      ...
      action: () =>
        cloneServerSizingRules({
          id,
          versionLabel: `${versionLabel.trim() || "next"}-${Date.now()}`,
        }),
      errorTitle: "规则集克隆失败",
    });
  }
    ...
    <div className="space-y-5">
      <div className="rounded-md border border-border/70 p-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">创建规则草稿</h2>
        </div>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          公式由代码固定；CMS 只允许保存经过 checksum 校验的 JSON 规则参数。
        </p>
        <div className="mt-4 grid gap-3 lg:grid-cols-[260px_minmax(0,1fr)_auto] lg:items-end">
          ...
          <Button
            type="button"
            onClick={createDraft}
            disabled={!parsed || isPending("server-sizing-rule:create")}
          >
                    ...
                    {rule.status === "draft" ? (
                      <>
                        <Button
                          ...
                          onClick={() => review(rule)}
                          disabled={isPending(
                            `server-sizing-rule:review:${rule.id}`,
                          )}
                        >
                        ...
                        <Button
                          ...
                          disabled={
                            !rule.reviewedBy ||
                            isPending(`server-sizing-rule:publish:${rule.id}`)
                          }
                        >
                          <Send className="size-4" />
                          发布
                      ...
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        onClick={() => clone(rule.id)}
                        disabled={isPending(
                          `server-sizing-rule:clone:${rule.id}`,
                        )}
                      >
...
function parseRules(
  value: string,
): Omit<ServerSizingRuleSet, "checksum"> | null {
  try {
