...
function affiliateParamLabel(input: {
  ...
  return input.affValue ? `${input.affParam}=${input.affValue}` : input.affParam;
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        ...
        <Badge
          variant={report.unmatchedLinks.length > 0 ? "destructive" : "outline"}
        >
          未命中 {report.unmatchedLinks.length}
        </Badge>
        <Badge
          ...
          <Table>
            ...
            <TableBody>
              {matchedLinks.map((item, index) => (
                <TableRow key={`${item.finalHref}-${index}`} className="align-top">
                  <TableCell>
      ...
      {report.matchedLinks.length > matchedLinks.length ? (
        <p className="text-xs text-muted-foreground">
          还有 {report.matchedLinks.length - matchedLinks.length} 条命中记录未展示。
        </p>
      ) : null}
