...
export function NetworkExperienceRuleManager({ rules }: { rules: RuleRow[] }) {
  ...
  function review(rule: RuleRow) {
    void mutate({
      key: `network-experience-rule:review:${rule.id}`,
  ...
  function publish(rule: RuleRow) {
    void mutate({
      key: `network-experience-rule:publish:${rule.id}`,
      pendingMessage: "正在发布线路经验规则...",
      action: () =>
        publishNetworkExperienceRules({
          id: rule.id,
          expectedRevision: rule.revision,
  ...
  function clone(rule: RuleRow) {
    void mutate({
      key: `network-experience-rule:clone:${rule.id}`,
      pendingMessage: "正在克隆规则集...",
      action: () =>
        cloneNetworkExperienceRules({
          id: rule.id,
          versionLabel: `${rule.versionLabel}-next`,
        }),
      errorTitle: "规则克隆失败",
    });
  }
    ...
    <div className="space-y-5">
      <div className="rounded-md border border-border/70 p-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">创建经验规则草稿</h2>
        </div>
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          规则只保存受控枚举和 reason
          code；必须包含风险码与验证码，不接受延迟、丢包、质量分或商家排名。
        </p>
        <div className="mt-4 grid gap-3 lg:grid-cols-[260px_minmax(0,1fr)_auto] lg:items-end">
          <div className="space-y-1.5">
            <Label className="text-xs">版本标签</Label>
            <Input
              value={versionLabel}
              onChange={(event) => setVersionLabel(event.target.value)}
              placeholder="2026.08-experience-v1"
            />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">规则 JSON</Label>
            <Textarea
              value={configText}
          ...
          <Button
            type="button"
            onClick={createDraft}
            disabled={!parsed || isPending("network-experience-rule:create")}
          >
            创建 draft
          </Button>
        </div>
        {!parsed ? (
          <p className="mt-2 text-xs text-destructive">
            规则 JSON 无效，需包含版本、引擎、schema 和 rules。
          </p>
        ) : null}
      </div>
      <div className="overflow-x-auto rounded-md border border-border/70">
        <table className="cms-mobile-sticky-actions w-full min-w-[900px] text-left text-sm">
          ...
          <tbody className="divide-y divide-border/70">
            {rules.map((rule) => (
              <tr key={rule.id}>
                <td data-mobile-label="版本" className="px-3 py-3 font-medium">
                  {rule.versionLabel}
                ...
                <td
                  ...
                  className="max-w-[260px] break-all px-3 py-3 font-mono text-xs text-muted-foreground"
                >
                  {rule.checksum}
                </td>
                <td
                ...
                >
                  {rule.reviewedBy ?? "未审核"} / {rule.publishedBy ?? "未发布"}
                </td>
                <td data-mobile-label="操作" className="px-3 py-3">
                    ...
                    {rule.status === "draft" ? (
                      <>
                        <Button
                          ...
                          onClick={() => review(rule)}
                          disabled={isPending(
                            `network-experience-rule:review:${rule.id}`,
                          )}
                        >
                        ...
                        <Button
                          ...
                          disabled={
                            !rule.reviewedBy ||
                            isPending(
                              `network-experience-rule:publish:${rule.id}`,
                            )
                          }
                        >
                          发布
                      ...
                      <Button
                        ...
                        onClick={() => clone(rule)}
                        disabled={isPending(
                          `network-experience-rule:clone:${rule.id}`,
                        )}
                      >
...
function parseSnapshot(value: string): NetworkExperienceRuleSetSnapshot | null {
  try {
    ...
    if (
      ...
      record.engineVersion !== "network-experience-engine-v1" ||
    ...
    } as unknown as NetworkExperienceRuleSetSnapshot;
  } catch {
    return null;
  }
}
