...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await pullNetworkMeasurementTask({
      method: request.method,
      path: new URL(request.url).pathname || NETWORK_PULL_PATH,
      headers,
      body,
    });
    ...
    console.error("Network measurement task pull failed:", error);
    return json({ error: "pull_unavailable" }, 503);
  }
}
